...
from great_expectations.execution_engine.ponder_batch_data import PonderBatchData
from great_expectations.execution_engine.split_and_sample.ponder_data_sampler import (
    PonderDataSampler,
)
from great_expectations.execution_engine.split_and_sample.ponder_data_splitter import (
    PonderDataSplitter,
)
...
@public_api
class PonderExecutionEngine(ExecutionEngine):
    """PonderExecutionEngine instantiates the ExecutionEngine API to support computations using Modin.
    ...
    def __init__(self, *args, **kwargs) -> None:
        ...
        self._data_splitter = PonderDataSplitter()
        self._data_sampler = PonderDataSampler()
    ...
    def load_batch_data(
        self, batch_id: str, batch_data: Union[PonderBatchData, pd.DataFrame]
    ) -> None:
        if isinstance(batch_data, pd.DataFrame):
            batch_data = PonderBatchData(self, batch_data)
        elif not isinstance(batch_data, PonderBatchData):
            raise gx_exceptions.GreatExpectationsError(
                "PonderExecutionEngine requires batch data that is either a DataFrame or a PonderBatchData object"
            )
...
"""
            ...
            elif isinstance(batch_spec.batch_data, PonderBatchData):
                df = batch_spec.batch_data.dataframe
            else:
                raise ValueError(
                    "RuntimeDataBatchSpec must provide a Pandas DataFrame or PonderBatchData object."
                )
            ...
            if self._s3 is None:
                raise gx_exceptions.ExecutionEngineError(
                    """PonderExecutionEngine has been passed a S3BatchSpec,
...
not {batch_spec.__class__.__name__}"""
        ...
        typed_batch_data = PonderBatchData(execution_engine=self, dataframe=df)
    ...
    def dataframe(self) -> pd.DataFrame:
        ...
        return cast(PonderBatchData, self.batch_manager.active_batch_data).dataframe
    ...
    def get_domain_records(  # noqa: C901 - 17
        ...
        if batch_id is None:
            ...
            if self.batch_manager.active_batch_data_id is not None:
                data = cast(
                    PonderBatchData, self.batch_manager.active_batch_data
                ).dataframe
            else:
            ...
            if batch_id in self.batch_manager.batch_data_cache:
                data = cast(
                    PonderBatchData, self.batch_manager.batch_data_cache[batch_id]
                ).dataframe
            else:
                raise gx_exceptions.ValidationError(
